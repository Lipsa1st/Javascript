...
function jsonConverter(fileName)
{
    //split the file as line by line
        ...
        {
            var column = line.split(",");//split columns by seperationg with comma
            if(index === 0)
            {
                headers = column;
            }
            if(line !== '' && index !== 0)
            {
                ...
                if( column[4] == "Total" && column[5] !== "All ages" )
                {
                    ...
                    {
                        ageGroup = column[5];
                        ageWise[ageGroup].total = ageWise[ageGroup].total +totalLiterate;
                    }
                    else
                    {
                        ageGroup=column[5];
                ...
                for(var i = 15; i <=43 ; i+=3)
                {
                    ...
                    {
                        educationCategory[eduTitle].total = educationCategory[eduTitle].total + parseInt(column[i]);
                    }
                    else
                    {
...
fs.writeFile('age.json',JSON.stringify(dataValue(ageWise)));//writes the file in age.json
fs.writeFile('education.json',JSON.stringify(dataValue(educationCategory)));//writes the file in educationCategory

 console.log("Data Length of Age wise Literates:"+dataValue(ageWise).length);//data length
 console.log("Age Wise : ");
 console.log(dataValue(ageWise)) ;

console.log("Data Length of Education Category :"+dataValue(educationCategory).length) ;
console.log("Education Category Wise : ");
console.log(dataValue(educationCategory)) ;
